        ...
        <section id="container" >
            <div class="row">
                <div class="col-sm-12">
                    ...
                    @endforeach
                    {{ Form::open(array('url'=>url('/login'), 'class'=>'form-signin form-signin-max-width')) }}
                    	<h2 class="form-signin-heading">Faça o Login</h2>
                        <div class="login-wrap">
                            <div class="user-login-info">
                                {{ Form::text('user', null, array('class'=>'form-control', 'placeholder'=>'Usuário')) }}
                                {{ Form::password('password', array('class'=>'form-control', 'placeholder'=>'Senha')) }}
                                <a href="{{url('/forgotPassword')}}">Esqueci a senha</a>
                            </div>
                            {{ Form::submit('Login', array('class'=>'btn btn-lg btn-login btn-block'))}}
                    	</div>
                    {{ Form::close() }}
                </div>
            </div>
